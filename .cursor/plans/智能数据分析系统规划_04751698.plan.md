...
- 大模型接入模块 [backend/app/llm/client.py](backend/app/llm/client.py)：使用百炼 OpenAI 兼容端点接入 Qwen3。
  - 端点 `https://dashscope.aliyuncs.com/compatible-mode/v1`，用 `langchain_openai.ChatOpenAI(model="qwen-plus"/"qwen3-...", base_url=..., api_key=...)`；备选 `langchain_community.chat_models.ChatTongyi`。
- 数据层
...
- NL2SQL 模块 [backend/app/agents/sql_agent.py](backend/app/agents/sql_agent.py)：基于 LangChain `create_sql_query_chain` 或 SQL Agent，结合表结构 schema 与会话上下文生成 SQL，执行并返回结果集（含只读/防注入约束，仅允许 SELECT）。
...
- LLM 接入：优先百炼 OpenAI 兼容模式，便于 LangChain 复用与流式输出。
- NL2SQL：仅允许只读 SELECT；将表结构 + 少量样例行注入 prompt 提升准确率。
...
- 模型与智能体：[llm/client.py](backend/app/llm/client.py)（接入 Qwen3）、[agents/sql_agent.py](backend/app/agents/sql_agent.py)（NL2SQL，仅 SELECT）、[agents/chart_agent.py](backend/app/agents/chart_agent.py)（生成并校验 ECharts option）。
- 记忆与接口：[memory/history.py](backend/app/memory/history.py)（按 session_id 上下文记忆）、[api/sessions.py](backend/app/api/sessions.py)（会话 CRUD）、[api/chat.py](backend/app/api/chat.py)（`POST /api/chat` SSE，事件 `sql`/`token`/`chart`/`done`）。
- 测试验收：用 `/docs` 或 `curl` 单测各接口——会话 CRUD 正常；给定自然语言能生成正确 SQL、返回结果集与合法 ECharts JSON；SSE 能按事件顺序流式输出；上下文记忆生效。
