...
- 测试验收：先跑 [test_chat_qwq.py](data-analysis/backend/test_chat_qwq.py)、[test_nl2sql_agent_qwq.py](data-analysis/backend/test_nl2sql_agent_qwq.py) 确认字段未漂移；再用 `/docs` 或 `curl` 单测——会话 CRUD、NL→SQL→结果→图表 JSON、SSE 事件顺序、多轮 `tool_call_id` 记忆。
